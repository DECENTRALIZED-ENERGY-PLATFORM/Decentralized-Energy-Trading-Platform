...
const db = mysql.createConnection({
  ...
  app.post("/create", (req, res) => {
    ...
    db.query(
      ...
      (err, result) => {
        ...
        res.status(201).json({ message: "Values Inserted" }); // Use 201 for created resource
      }
    );
  });
  ...
  app.put("/updateContract1", (req , res)=>{
    ...
    db.query(sql,[transporter,hashTX3,mtTX3,adress_contract] ,(err, result)=>{ 
      if (err) {
        console.error(err); // Change console.log to console.error for better clarity
        return res.status(500).json({ error: "Database query error" });
      }
      return res.json(result);
     });
...
//     return res.status(500).json({ error: "Internal Server Error" }); // Send a response to the client
// }
